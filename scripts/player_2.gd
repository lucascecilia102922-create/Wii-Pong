...
func move(delta:float) -> void:
	if is_instance_valid(ball) and has_node("../Ball/StaticBody2D"):
		...
		var ball_position = move_toward(global_position.x, ball_pos.x, delta * Speed)
		global_position.x = ball_position
	
	else:
		var Speed = 250
		var ball_position = move_toward(global_position.x, initial_pos.x, delta * Speed)
		global_position.x = ball_position
